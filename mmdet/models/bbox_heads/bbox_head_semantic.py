...
class BBoxSemanticHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.seen_class = seen_class # seen_class is False
        self.gzsd = gzsd # gzsd False
        self.reg_with_semantic = reg_with_semantic # False
        self.share_semantic = share_semantic # False
        self.with_avg_pool = with_avg_pool # False
        self.with_reg = with_reg # True
        self.with_semantic = with_semantic # True
        self.roi_feat_size = _pair(roi_feat_size) # (7, 7)
        self.roi_feat_area = self.roi_feat_size[0] * self.roi_feat_size[1]
        self.in_channels = in_channels # 256
        self.num_classes = num_classes # 66
        self.target_means = target_means # [0.0, 0.0, 0.0, 0.0]
        self.target_stds = target_stds # [0.1, 0.1, 0.2, 0.2]
        self.reg_class_agnostic = reg_class_agnostic # True
        ...
        if self.with_semantic:
            ...
            # shape
            # self.voc.shape (300, 4717)
            # self.vec.shape (300, 66)
            # self.vec_unseen.shape (300, 16)
            if self.voc is not None:
                # self.kernel_semantic is (4717, 300) 
